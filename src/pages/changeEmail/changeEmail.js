/*
 * @Author: your name
 * @Date: 2021-06-29 10:53:30
 * @LastEditTime: 2021-06-29 15:12:02
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /mesonweb/src/pages/changeEmail/changeEmail.js
 */

import React from "react";
import AdminLayout from "../../components/layout/adminLayout";
import PhoneInput from "react-phone-input-2";
import "react-phone-input-2/lib/style.css";
//import "./register.css";
import SendCode from "../../components/sendcode/sendcode";
import axios from "axios";
import { withAlert } from "react-alert";
import UserTypeSelector from "../../components/usertypes/usertypeselector";
import UserManager from "../../manager/usermanager";
import Global from "../../global/global";
import Captcha from "react-captcha-code";
import Utils from "../../utils/utils"
class ChangeEmail extends React.Component {
  constructor(props) {
    super(props);
    this.existEmail="";
    this.newEmail=""
    this.existEmailVcode = "";
    this.newEmailVcode="";
    this.username = "";
    this.captchaId = 0;
    this.inputCode = "";
    this.captchaRef = null;
    this.captchaCode = "";

    this.state={
      captchaBase64: "data:image/png;base64,"
    }
  }

  async GetCaptchaFromServer(){
    let url = "/api/v1/user/getcaptcha"
    axios
      .get(Global.apiHost + url)
      .then((response) => {
        if (response&&response.data.status == 108){
          this.props.alert.error("Please wait at least 2 seconds before refresh");
          return;
        }
        if (response&&response.data.status == 0){
          this.setState({
            captchaBase64:response.data.data.base64Code
          })
          this.captchaId=response.data.data.id
          return;
        }
      })
  }

  async componentDidMount() {
    if (UserManager.GetUserInfo() == null) {
      await UserManager.UpdateUserInfo();
    }
    this.GetCaptchaFromServer()
  }

  checkCaptcha() {
    // if (this.captchaCode != this.inputCode) {
    //   this.props.alert.error("please input correct captcha");
    //   return false;
    // }
    if (this.inputCode.length!=4) {
      this.props.alert.error("please input correct captcha");
      return false;
    }
    return true;
  }

  checkExistEmail(){
    if (this.existEmail.length<=4) {
      this.props.alert.error("please input correct email address");
      return false;
    }

    let reg=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
    let pattern = new RegExp(reg)

    if (!pattern.test(this.existEmail)){
      this.props.alert.error("please input correct email address");
      return false;
    }

    return true;
  }

  checkNewEmail(){
    if (this.newEmail.length<=4) {
      this.props.alert.error("please input correct email address");
      return false;
    }

    let reg=/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
    let pattern = new RegExp(reg)

    if (!pattern.test(this.newEmail)){
      this.props.alert.error("please input correct email address");
      return false;
    }

    return true;
  }

  checkvcode() {
    if (this.newEmailVcode.length<4||this.newEmailVcode.length>6) {
      this.props.alert.error("please input correct new email vcode");
      return false;
    }
    if (this.existEmailVcode.length<4||this.existEmailVcode.length>6) {
        this.props.alert.error("please input correct exist email vcode");
        return false;
      }
    return true;
  }

  clickSendExistEmailVcode(){
    if (!this.checkExistEmail()) {
      return;
    }

    axios
      .post(Global.apiHost + "/api/v1/user/resetgetemailvcode", {
        email: this.existEmail
      })
      .then((response) => {
        //console.log(response);

        if (response&&response.data.status == 2006){
            this.props.alert.error("Email format is not correct");
            return;
          }
  
          if (response.data.status == 2102) {
            this.props.alert.error("Email not exist");
            return;
          }
  
          if (response && response.data.status == 102) {
            this.props.alert.error(
              "please wait 60 seconds before you send verify code again"
            );
            return
          }
  
          if (response && response.data.status == 0) {
            this.props.alert.success(
              "VCode send"
            );
            return
          }
      });

  }


  clickSendNewEmailVcode(){
    if (!this.checkNewEmail()) {
      return;
    }

    axios
      .post(Global.apiHost + "/api/v1/user/registergetemailvcode", {
        email: this.newEmail
      })
      .then((response) => {
        //console.log(response);

        if (response&&response.data.status == 2006){
          this.props.alert.error("Email format is not correct");
          return;
        }

        if (response.data.status == 2002) {
          this.props.alert.error("Email already exist");
          return;
        }

        if (response && response.data.status == 102) {
          this.props.alert.error(
            "please wait 60 seconds before you send verify code again"
          );
          return
        }

        if (response && response.data.status == 0) {
          this.props.alert.success(
            "VCode send"
          );
          return
        }
      });

  }

  changeEmail() {
    if (!this.checkCaptcha()) {
      return;
    }


    if(!this.checkExistEmail()){
      return
    }

    if(!this.checkNewEmail()){
        return
      }

    if (!this.checkvcode()) {
      return;
    }

    let userInfo=UserManager.GetUserInfo()
    let userName=userInfo?userInfo.username:""

    this.GetCaptchaFromServer()

    axios
      .post(Global.apiHost + "/api/v1/user/changeemail", {
        username: userName,
        existEmail:this.existEmail,
        newEmail:this.newEmail,
        existEmailVcode : this.existEmailVcode,
        newEmailVcode:this.newEmailVcode,
        captcha:this.inputCode,
        captchaId:this.captchaId,
      })
      .then((response) => {
        switch (response.data.status) {
          case 107:
            this.props.alert.error("Captcha wrong");
            return;
          case 2001:
            this.props.alert.error("User already exist!");            
            return;
          case 2002:
            this.props.alert.error("Email already exist!");
            return;
          case 2003:
            this.props.alert.error("Phone already exist!");
            return;
          case 2004:
            this.props.alert.error("verify code wrong!");
            return;
          case 2005:
            this.props.alert.error("User type error!");
            return;
          default:
            break;
        }

        if (response.data.status == 0) {
            this.props.alert.success("New Email Bind Success");
          return;
        }

        //console.log(response);
        this.props.alert.error("some thing wrong,please contact us!");
      });
  }

  render() {
    return (
      <AdminLayout name="Register" description="Register page">
        <div className="card border-light shadow-sm">
          <div className="card-body" style={{ color: "#555e68" }}>
            <form style={{ textAlign: "left" }}>
              
              {/* exist email */}
              <div className="form-group">
              <label>Exist Email</label>
              <input
                type="email"
                className="form-control"
                onChange={(event) => {
                  this.existEmail = event.target.value.trim();
                }}
                aria-describedby="emailHelp"
                placeholder="Enter Exist Email"
              />
              </div>

              <div className="form-row">
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">Exist email code</label>
                    <input
                      className="form-control py-3"
                      type="text"
                      placeholder="Enter vcode"
                      onChange={(event) => {
                        this.existEmailVcode = event.target.value.trim();
                      }}
                    ></input>
                  </div>
                </div>
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">send email</label>
                    <div style={{ textAlign: "left" }}>
                      <SendCode
                        checkphonecorrect={() => {
                          return this.checkExistEmail();
                        }}
                        click={() => {
                          this.clickSendExistEmailVcode();
                        }}
                      ></SendCode>
                    </div>
                  </div>
                </div>
              </div>

            {/* new email */}
              <div className="form-group">
              <label>New Email</label>
              <input
                type="email"
                className="form-control"
                onChange={(event) => {
                  this.newEmail = event.target.value.trim();
                }}
                aria-describedby="emailHelp"
                placeholder="Enter Email"
              />
              </div>

              <div className="form-row">
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">New email code</label>
                    <input
                      className="form-control py-3"
                      type="text"
                      placeholder="Enter vcode"
                      onChange={(event) => {
                        this.newEmailVcode = event.target.value.trim();
                      }}
                    ></input>
                  </div>
                </div>
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">send email</label>
                    <div style={{ textAlign: "left" }}>
                      <SendCode
                        checkphonecorrect={() => {
                          return this.checkNewEmail();
                        }}
                        click={() => {
                          this.clickSendNewEmailVcode();
                        }}
                      ></SendCode>
                    </div>
                  </div>
                </div>
              </div>

              <div className="form-row">
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">Captcha</label>
                    <input
                      className="form-control py-3"
                      type="text"
                      placeholder="Enter captcha"
                      onChange={(event) => {
                        this.inputCode = event.target.value.trim();
                      }}
                    ></input>
                  </div>
                </div>
                <div className="col-md-2">
                  <div className="form-group">
                    <label className="small mb-2">click to refresh</label>
                    <div style={{ textAlign: "left" }}>
                      <img src={this.state.captchaBase64} alt="click to refresh" onClick={()=>{
                        this.GetCaptchaFromServer()
                      }}/>
                    </div>
                  </div>
                </div>
              </div>


              <div className="form-group mt-4 mb-0">
                <div
                  className="btn btn-primary-rocket btn-block"
                  onClick={() => {
                    this.changeEmail();
                  }}
                  href="#"
                >
                  Change Email
                </div>
              </div>
            </form>
          </div>
        </div>
      </AdminLayout>
    );
  }
}

export default withAlert()(ChangeEmail);

//export  default  RegisterPage;
